  ...
  def create
    ...
    respond_to do |format|
      if @mentor.save
        ...
        #send email to user (mentor)
        UserMailer.mentor_email(@mentor).deliver
        
        format.json { render :json => @mentor , :status => :ok }
      else
        format.js { render :json => @mentor.error, :status => :unprocessable_entity }
      end
    end
  end
